"""Tests for FastAPI endpoints using TestClient (pipeline mocked)."""
...
def client(monkeypatch):
    ...
    monkeypatch.setenv("CHAT_MODE", "rag")
    monkeypatch.setattr(state, "pipeline", fake_pipeline)
    monkeypatch.setattr(state, "chat_service", ChatService(pipeline=fake_pipeline))
    # Force rag mode on the service
    monkeypatch.setattr(
        "app.services.chat.config.CHAT_MODE",
        "rag",
    )
...
def test_query_validation_short_question(client):
    ...
    r2 = client.post("/query", json={"question": "   "})
    assert r2.status_code == 422

    r3 = client.post("/query", json={"question": "hi"})
...
def test_dummy_query(monkeypatch):
    import app.main as api_main
    import app.state as state

    monkeypatch.setattr("app.services.chat.config.CHAT_MODE", "dummy")
    monkeypatch.setattr(state, "chat_service", ChatService(pipeline=None))
    client = TestClient(api_main.app)
    r = client.post("/query", json={"question": "What is FinQuery?"})
    assert r.status_code == 200
    body = r.json()
    assert body["model"] == "dummy"
    assert "dummy reply" in body["answer"].lower()
