...
def _upload(auth_client, company_id: int) -> int:
...
def test_process_flow_uploaded_to_processed(
    auth_client, db_session, company_factory, isolated_storage, stubbed_runner
):
    from app.models.document import Document

    company = company_factory("PRCS")
    document_id = _upload(auth_client, company.id)

    before = auth_client.get(f"/api/v1/documents/{document_id}/status")
    assert before.status_code == 200
    assert before.json()["status"] == "uploaded"

    queued = auth_client.post(f"/api/v1/documents/{document_id}/process")
    assert queued.status_code == 202
    assert auth_client.get(f"/api/v1/documents/{document_id}/status").json()[
        "status"
    ] == "queued"
    ...
    after = auth_client.get(f"/api/v1/documents/{document_id}/status")
    body = after.json()
...
def test_failed_status_carries_error(
    ...
    document_id = _upload(auth_client, company.id)
    auth_client.post(f"/api/v1/documents/{document_id}/process")
    ...
    body = auth_client.get(f"/api/v1/documents/{document_id}/status").json()
...
def test_process_unknown_document_404(auth_client):
    response = auth_client.post("/api/v1/documents/999999/process")
